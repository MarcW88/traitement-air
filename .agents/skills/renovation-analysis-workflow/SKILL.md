...
# Renovation Analysis Workflow v2
...
- `renovation-analysis-workflow` = recherche, diagnostic, décision, cluster audit et publish review ;
- `renovation-content-workflow` = brief, production/correction et post-write gap check après handoff valide.
...
Gate final après `renovation-content-workflow`.
...
`DEEP_REWRITE` peut passer à `renovation-content-workflow` seulement si :
...
- `/renovatie-plannen/` = décisions transversales : démarrage, ordre, phases, budget, coûts, permis, aides, rendement ;
- `/renovatieprojecten/` = décision/préparation d'un chantier concret ;
- `/verduurzamen/` = énergie, confort, installations et conditions de pertinence ;
- `/problemen-oplossen/` = symptôme, causes possibles, contrôles sûrs et prochaine action ;
- `/doe-het-zelf/` = uniquement tâches réellement exécutables et sûres pour le public ;
- `/vakman-en-offertes/` = sélection, scope, comparaison, contractualisation et conversion.
